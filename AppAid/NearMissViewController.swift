...
class NearMissViewController: UIViewController {
        ...
        if let year = chooseYearButton.titleLabel?.text{
            if let injuryType = chooseInjuryTypeButton.titleLabel?.text{
                if let intYear = Int(year){
                    ...
                    view.addSubview(myActivityIndicator)
                    let myUrl = URL(string: "http://ec2-54-71-121-51.us-west-2.compute.amazonaws.com:8080/appaid-ws/api/stat/nearmiss?year=\(intYear)&type=\(injury)")
                    var request = URLRequest(url:myUrl!)
                    request.httpMethod = "GET"
                    request.addValue("application/json", forHTTPHeaderField: "Accept")
